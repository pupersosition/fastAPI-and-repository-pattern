...
async def create_manufacturer_endpoint(manufacturer: ManufacturerCreate, manufacturer_repo: AbstractManufacturerRepository = Depends(get_manufacturer_repo)):
    return await manufacturer_repo.create(manufacturer)

@router.get("/{manufacturer_id}", response_model=Manufacturer)
async def read_manufacturer(manufacturer_id: int, manufacturer_repo: AbstractManufacturerRepository = Depends(get_manufacturer_repo)):
    db_manufacturer = await manufacturer_repo.get_by_id(manufacturer_id)
    if db_manufacturer is None:
        raise HTTPException(status_code=404, detail="Manufacturer not found")
    return db_manufacturer

@router.get("/", response_model=list[Manufacturer])
async def list_manufacturers(skip: int = 0, limit: int = 10, manufacturer_repo: AbstractManufacturerRepository = Depends(get_manufacturer_repo)):
    return await manufacturer_repo.get_all(skip=skip, limit=limit)

@router.put("/{manufacturer_id}", response_model=Manufacturer)
async def update_manufacturer_endpoint(manufacturer_id: int, manufacturer: ManufacturerUpdate, manufacturer_repo: AbstractManufacturerRepository = Depends(get_manufacturer_repo)):
...
async def delete_manufacturer_endpoint(manufacturer_id: int, manufacturer_repo: AbstractManufacturerRepository = Depends(get_manufacturer_repo)):
